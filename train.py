...
def main(args):
    ...
    trainer = Trainer(
        ...
        g_loss=config['training']['g_weight'],
        d_loss=config['training']['d_weight'],
    ...
    while not out:
        ...
        for x_real, _ in train_loader:
            ...
            # Print stats
            if (it % config['training']['print_every']) == 0:
                g_loss_last = logger.get_last('losses', 'generator')
                d_loss_last = logger.get_last('losses', 'discriminator')
                e_loss_last = logger.get_last('losses', 'encoder')
                d_reg_last = logger.get_last('losses', 'regularizer')

                print('[epoch %0d, it %4d] g_loss = %.4f, d_loss = %.4f, e_loss = %.4f, reg=%.4f'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dvae_name', default='none', type=str,
                        help='Name of the experiment for pre-training disentangling VAE')
